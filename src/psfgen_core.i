...
%typemap(in) const topo_mol_ident_t *target {
  PyObject *tmpobj;
...
/* These two arguments are optional to their functions so make them
 * default to NULL */
%typemap(default) FILE * namdbinfile {
    $1 = NULL;
}
%typemap(default) FILE * velnamdbinfile {
    $1 = NULL;
}

/* Make it shut up about const char* maybe leaking memory */
